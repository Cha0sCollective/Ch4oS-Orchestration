...
export function createMcpServer(service: WorkerService): McpServer {
  ...
  server.registerTool('start_task', { description: 'Start a bounded worker job using an approved local or free remote model. Reads scoped snapshots and optionally runs enabled isolated checks. Returns proposals; never applies edits. Nemotron Ultra requires no qualification; use mode work. Results require Codex verification.', inputSchema: taskRequestSchema, annotations: { readOnlyHint: false, destructiveHint: false, openWorldHint: remote } }, request => result(() => service.startTask(request)));
