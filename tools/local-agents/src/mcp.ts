...
export function createMcpServer(service: WorkerService): McpServer {
  ...
  server.registerTool('capabilities', { description: 'Inspect worker readiness, registered repository scope, qualified task classes, and limits before delegating.', inputSchema: z.object({}).strict(), annotations: { readOnlyHint: true, openWorldHint: remote } }, () => result(() => service.capabilities()));
  server.registerTool('start_task', { description: 'Start a bounded worker job using an approved local or free remote model. Reads scoped snapshots and optionally runs enabled isolated checks. Returns proposals; never applies edits. Results require Codex verification.', inputSchema: taskRequestSchema, annotations: { readOnlyHint: false, destructiveHint: false, openWorldHint: remote } }, request => result(() => service.startTask(request)));
